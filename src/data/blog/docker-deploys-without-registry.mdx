...
The obvious fix is a Docker registry: push once, pull anywhere. I considered running a private registry on the same server, but that reintroduced complexity I was trying to avoid (auth, storage, ports, and more moving pieces). I wanted to keep Haloy as a single binary with minimal dependencies.
