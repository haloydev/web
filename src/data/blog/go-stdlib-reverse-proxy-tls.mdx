...
Then I rebuilt it on Go's stdlib and it came together faster than expected. `httputil.ReverseProxy` handles proxying, `tls.Config.GetCertificate` handles dynamic cert loading, and `golang.org/x/crypto/acme` talks to Let's Encrypt directly. Certificate issuance is faster than in the previous setups, and everything now runs inside `haloyd` with no extra containers, generated config files, or reload hooks.
