---
title: "Reverse Proxy with Auto TLS Using Go's Stdlib"
slug: 'go-stdlib-reverse-proxy-tls'
description: 'How Haloy uses Go''s httputil.ReverseProxy and tls.Config to build a production reverse proxy with automatic Let''s Encrypt certificates and minimal dependencies.'
pubDate: 2026-02-13
...
Haloy's reverse proxy and certificate manager were not always stdlib-based. I tried two earlier architectures first, and walking through that progression explains why the current approach stuck.

**Update:** Haloy now runs the proxy as a separate `haloy-proxy` daemon. The proxy implementation described here is still the same stdlib-based design, but it no longer lives in the same process as `haloyd`; `haloyd` pushes routing snapshots to the proxy so control-plane restarts do not interrupt traffic.
...
Both approaches felt bolted on. Haloy is supposed to be one Go binary running as a systemd service (or rc service on Alpine), so depending on extra containers and tool-specific config did not fit the design.

Then I rebuilt it on Go's stdlib and it came together faster than expected. `httputil.ReverseProxy` handles proxying, `tls.Config.GetCertificate` handles dynamic cert loading, and `golang.org/x/crypto/acme` talks to Let's Encrypt directly. Certificate issuance is faster than in the previous setups, and the current version runs as native Haloy services with no extra containers, generated config files, or reload hooks.
...
I started this project expecting to lean on nginx for the proxy layer and a library like `lego` for certificate management. The stdlib alternative is easier to reason about, has fewer moving parts, and compiles into small Go binaries with no proxy runtime dependencies.
