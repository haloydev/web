---
title: 'Haloy vs Kubernetes: Do You Need an Orchestrator?'
slug: 'haloy-vs-kubernetes'
description: 'Kubernetes is the standard for container orchestration, but most apps don''t need it. A practical look at what each tool does and how to decide.'
pubDate: 2026-02-16
...
This comes up a lot, so here's the direct answer: how does Haloy compare to Kubernetes?

Short version: they solve different problems at different scales. Kubernetes is a distributed orchestrator for complex systems. Haloy is a deployment tool for running Docker apps on your own servers with minimal setup. Comparing them directly is like comparing a commercial kitchen and a home oven. Both cook food, but they are built for different jobs.

Still, both tools run containers, so the comparison is fair. Here's a practical breakdown.

## Quick Comparison

| | Kubernetes | Haloy |
|---|---|---|
| **Architecture** | Distributed control plane (API server, etcd, scheduler, kubelet, kube-proxy) | Single binary CLI + lightweight server daemons |
...
| **Resource overhead** | Significant (control plane needs 2+ GB RAM minimum) | Minimal (small Go binaries) |
...
Haloy is a CLI deployment tool paired with lightweight server daemons. You write a YAML config file in your project root, run `haloy deploy`, and it builds a Docker image locally, uploads the changed layers to your server, and does a zero-downtime swap. `haloyd` handles deployments, certificates, health checks, and container management; `haloy-proxy` handles public HTTP/HTTPS traffic.

There's no cluster, no distributed state, and no heavyweight platform. One binary runs on your laptop, and small native services run on your server.
...
Haloy includes a built-in reverse proxy with automatic Let's Encrypt certificate provisioning. Point your DNS at the server, set your `domains` in config, and HTTPS works on the first deploy. No external proxy stack is needed.
...
Most developers don't need Kubernetes on day one. If you've ever set up a cluster for a project that could have lived on one or two servers, you already know the overhead is real. Keeping a cluster healthy, managing RBAC, debugging networking, and learning the ecosystem takes time that small teams often need elsewhere.
...
Haloy covers the common case: you have a Docker app, a server, and you want reliable deploys with HTTPS and zero downtime. If your needs outgrow that, Kubernetes is still there when you need it.
