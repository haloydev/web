...
Third-party analytics creates a pile of trade-offs at once: rising monthly cost, data on infrastructure you don't control, extra compliance overhead, and tracking scripts that get blocked by default in many browsers. If you already run your own servers, self-hosting can remove most of that friction.

This post compares three open-source options, Umami, Plausible Community Edition, and Rybbit, with Haloy deployment configs for each. I'll also cover practical ad-blocker workarounds and when it makes sense to move analytics to a separate server.
...
If you just want the quick comparison:
...
Then reference the proxied script in your HTML instead of the direct analytics URL. The browser sees a request to your own domain and path, which usually avoids common analytics blocklists.
...
These three tools land at different points on the spectrum:
...
For most people running a personal site or small SaaS, Umami on the same server is the easiest place to start. It's light and covers core web analytics. If you later need deeper product analytics, Plausible and Rybbit are solid next steps.
