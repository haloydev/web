...
description: 'Complete CLI reference for the haloy command. Covers deploy, status, logs, rollback, exec, server management, and more.'
---
...
Show or apply a prune plan for old deployment image tags on the server.

This command is target-aware and uses the target's resolved app name and server from your haloy config. By default it performs a dry run. Use `--yes` to actually remove tags.

<Code code={`
  # Dry run using the target's configured history keep count
  haloy prune-images --targets staging

  # Dry run with an explicit keep count
  haloy prune-images --targets staging --keep 2

  # Apply the prune plan
  haloy prune-images --targets staging --yes

  # Apply a more aggressive prune for all targets
  haloy prune-images --all --keep 1 --yes
`} lang="bash" />
...
<div class="list-unstyled">
- `--config, -c <path>` - Path to config file or directory (default: current directory)
- `--targets, -t <name>` - Prune images for specific target(s) (comma-separated for multiple)
- `--all, -a` - Prune images for all targets
- `--keep <n>` - Number of deployment images to keep locally (defaults to the target's `image.history` policy)
- `--yes` - Apply the prune plan instead of running a dry run
</div>

**Behavior:**

- Dry runs show the running deployment IDs and the image tags that would be removed.
- Applied runs remove only old deployment tags for the app, while avoiding the currently running image.
- This is useful when a deploy cannot proceed because the server is low on disk space before normal post-deploy cleanup runs.
...
Stream application container logs (stdout/stderr) in real-time, similar to `docker logs -f`. Shows the last N lines of history first, then follows new output until Ctrl+C (unless `--no-follow` is used).
...
- `--no-follow` - Print existing logs and exit without following
- `--raw` - Raw log lines with no prefixes or extra formatting
</div>
...
<Code code={`
  ...
  # Override the remote container port
  haloy tunnel -t myapp --port 15432 --remote-port 5432

  # Tunnel to a specific container (for apps with replicas)
  haloy tunnel -t myapp --port 15432 --container abc123
`} lang="bash" />
...
- `--port <port>` - Local port to listen on (default: port from config)
- `--remote-port <port>` - Remote container port to tunnel to (default: port from config)
...
Store registry credentials on a Haloy server. By default, the server URL and API token are read from `haloy.yaml`. haloyd uses these credentials when pulling images from that registry during deploys and rollbacks.

`$DOCKERHUB_TOKEN` and `$GHCR_TOKEN` below are example environment variable names. Set them yourself before running the command; Haloy only reads the token from stdin.

<Code code={`
  # Set the token in your local shell first
  export DOCKERHUB_TOKEN="paste-your-docker-hub-access-token"

  # Docker Hub, using the server from haloy.yaml
  printf '%s' "$DOCKERHUB_TOKEN" | haloy server registry login docker.io --username your-dockerhub-username --password-stdin

  # GitHub Container Registry
  export GHCR_TOKEN="paste-your-github-container-registry-token"
  printf '%s' "$GHCR_TOKEN" | haloy server registry login ghcr.io --username your-github-username --password-stdin

  # Multi-target config
  printf '%s' "$DOCKERHUB_TOKEN" | haloy server registry login docker.io --targets staging --username your-dockerhub-username --password-stdin

  # Direct server override
  printf '%s' "$DOCKERHUB_TOKEN" | haloy server registry login docker.io --server haloy.example.com --username your-dockerhub-username --password-stdin
`} lang="bash" />

The pipe before `haloy` is required when using `--password-stdin`. Running `haloy server registry login ... --password-stdin` by itself has no token to read; Haloy exits with an error instead of waiting for terminal input.

In CI, create `DOCKERHUB_TOKEN` or `GHCR_TOKEN` as a secret or environment variable in your CI provider, then use the same pipe command.

For Docker Hub, replace `your-dockerhub-username` with your actual Docker Hub username, not your email address, and use a Docker Hub access token. Docker's browser one-time-code login is a Docker CLI flow and is not used by `haloy server registry login`.
...
**Syntax:**
<Code code="haloy server registry login <registry> --username <username> (--password <password> | --password-stdin) [--server <url>] [--targets <targets> | --all]" lang="bash" />

**Arguments:**

<div class="list-unstyled">
- `<registry>` - Registry host, for example `docker.io`, `ghcr.io`, or `registry.example.com`
</div>
...
- `--password, -p <password>` - Registry password or access token
- `--password-stdin` - Read the registry password or access token from stdin
- `--server, -s <url>` - Server URL (overrides config file)
- `--config, -c <path>` - Path to config file or directory
- `--targets, -t <targets>` - Apply to specific target(s) in a multi-target config
- `--all, -a` - Apply to all targets in a multi-target config
</div>

Use `--password-stdin` in scripts to avoid passing tokens as command-line arguments.

For multi-target config files that resolve to one Haloy server, no target selector is required. If targets resolve to multiple servers, use `--targets` or `--all`.

These credentials are stored on the server at `/var/lib/haloy/registries.yaml` and are used only when the target does not define `image.registry`. Credentials in `haloy.yaml` take precedence, so a stale or incorrect `image.registry` block can override valid server-level credentials.

### server registry list

List registry credentials configured on a Haloy server. The password/token is never shown.

<Code code={`
  haloy server registry list
  haloy server registry list --targets staging
  haloy server registry list --server haloy.example.com
`} lang="bash" />

**Syntax:**
<Code code="haloy server registry list [--server <url>] [--targets <targets> | --all]" lang="bash" />
...
- `--server, -s <url>` - Server URL (overrides config file)
- `--config, -c <path>` - Path to config file or directory
- `--targets, -t <targets>` - List credentials for specific target server(s)
- `--all, -a` - List credentials for all target servers
</div>

### server registry logout

Remove registry credentials from a Haloy server.

<Code code={`
  haloy server registry logout docker.io
  haloy server registry logout ghcr.io --targets staging
  haloy server registry logout docker.io --server haloy.example.com
`} lang="bash" />

**Syntax:**
<Code code="haloy server registry logout <registry> [--server <url>] [--targets <targets> | --all]" lang="bash" />
...
Get the component versions from a Haloy server.

<Code code="haloy server version" lang="bash" />

Current servers report both the control plane and proxy when `haloy-proxy` is installed:
...
Stream platform-level operational logs from the `haloyd` server daemon in real-time. These are internal logs showing deployments, health checks, routing snapshot updates, etc. Proxy HTTP access logs (GET/POST requests) are hidden by default to keep the output focused on important platform events.
