...
For Docker Hub, use a Docker Hub access token with your Docker Hub username. Docker's browser one-time-code login is a Docker CLI flow and is not used by `haloy server registry login`.
...
These credentials are stored on the server at `/var/lib/haloy/registries.yaml` and are used only when the target does not define `image.registry`. Credentials in `haloy.yaml` take precedence.
