...
Store registry credentials on a Haloy server. haloyd uses these credentials when pulling images from that registry during deploys and rollbacks.

<Code code={`
  # Docker Hub
  echo "$DOCKERHUB_TOKEN" | haloy server registry login haloy.example.com docker.io --username your-dockerhub-username --password-stdin

  # GitHub Container Registry
  echo "$GHCR_TOKEN" | haloy server registry login haloy.example.com ghcr.io --username your-github-username --password-stdin

  # Custom registry
  haloy server registry login haloy.example.com registry.example.com --username deploy-bot --password-stdin
`} lang="bash" />

**Syntax:**
<Code code="haloy server registry login <server-url> <registry> --username <username> (--password <password> | --password-stdin)" lang="bash" />

**Arguments:**

<div class="list-unstyled">
- `<server-url>` - Haloy server URL
...
Use `--password-stdin` in scripts to avoid leaving tokens in shell history.
...
<Code code="haloy server registry list haloy.example.com" lang="bash" />

**Syntax:**
<Code code="haloy server registry list <server-url>" lang="bash" />
...
<Code code={`
  haloy server registry logout haloy.example.com docker.io
  haloy server registry logout haloy.example.com ghcr.io
`} lang="bash" />

**Syntax:**
<Code code="haloy server registry logout <server-url> <registry>" lang="bash" />
