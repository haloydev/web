---
title: 'Basic Configuration'
slug: 'basic-configuration'
section: configuration
description: 'Create a haloy.yaml configuration file for your application. Set up server URL, domains, environment variables, and deployment options.'
---

import Code from '@/components/code.astro';

Learn how to create a basic Haloy configuration file to deploy your application.

## Minimal Configuration

The simplest `haloy.yaml` configuration:

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  domains:
    - domain: "my-app.com"
      aliases:
        - "www.my-app.com"
`}
  lang="yaml"
/>

This configuration:

- Looks for a Dockerfile in the same directory
- Builds the image locally
- Uploads it to your server
- Deploys with SSL/TLS enabled
- Redirects www to the main domain

See [Domain Configuration](/docs/domain-configuration) for DNS setup, aliases, and multi-target domain examples.

## Format Support

Haloy supports YAML, JSON, and TOML for configuration files.

### YAML (`*.yml` or `*.yaml`)

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  domains:
    - domain: "my-app.com"
      aliases:
        - "www.my-app.com"
  health_check_path: "/health"
  min_ready_seconds: 10
`}
  lang="yaml"
/>

### JSON (`*.json`)

<Code
  code={`
  {
    "name": "my-app",
    "server": "haloy.yourserver.com",
    "domains": [
      {
        "domain": "my-app.com",
        "aliases": ["www.my-app.com"]
      }
    ],
    "healthCheckPath": "/health",
    "minReadySeconds": 10
  }
`}
  lang="json"
/>

### TOML (`*.toml`)

<Code code={`
  name = "my-app"
  server = "haloy.yourserver.com"
  health_check_path = "/health"
  min_ready_seconds = 10

[[domains]]
domain = "my-app.com"
aliases = ["www.my-app.com"]
`} lang="toml" />

**Naming Convention:**

- YAML/TOML: Use `snake_case` (e.g., `health_check_path`)
- JSON: Use `camelCase` (e.g., `healthCheckPath`, `minReadySeconds`)

## Adding a Domain

To make your app accessible via a domain:

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  domains:
    - domain: "my-app.com"
`}
  lang="yaml"
/>

For DNS records, aliases, and common troubleshooting steps, see [Domain Configuration](/docs/domain-configuration).

## Multiple Domains and Aliases

<Code
  code={`
  domains:
    - domain: "my-app.com"
      aliases:
        - "www.my-app.com"
        - "app.my-app.com"
    - domain: "another-domain.com"
`}
  lang="yaml"
/>

## Custom Port

If your application listens on a port other than 8080:

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  port: "3000"  # Your app's internal port
`}
  lang="yaml"
/>

The proxy will route traffic from ports 80/443 to this container port.

## Health Checks

Configure a custom health check endpoint:

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  health_check_path: "/api/health"
`}
  lang="yaml"
/>

Default: `"/"`

## Readiness Stabilization

If your app can pass its initial health checks and then fail a few seconds later, add `min_ready_seconds` to require a short stabilization window before Haloy marks the container ready:

<Code
  code={`
  name: "my-app"
  server: haloy.yourserver.com
  health_check_path: "/api/health"
  min_ready_seconds: 10
`}
  lang="yaml"
/>

Haloy waits until the container is healthy and has been running for at least the configured number of seconds.

- Default: `0` (disabled)
- Valid range: `0` to `600`
- JSON field name: `minReadySeconds`

## Configuration File Location

By default, `haloy` looks for configuration in the current directory:

- `haloy.yaml`
- `haloy.yml`
- `haloy.json`
- `haloy.toml`

Specify a custom config file:

<Code code={`haloy deploy --config /path/to/config.yaml`} lang="bash" />

## Next Steps

- [All Configuration Options](/docs/configuration-reference)
- [Domain Configuration](/docs/domain-configuration)
- [Set up Environment Variables](/docs/environment-variables)
- [Configure Volumes](/docs/volumes)
- [Image Configuration](/docs/image-configuration)
- [Agent Skills](/docs/agent-skills) - Generate Dockerfiles and configs with AI assistants
