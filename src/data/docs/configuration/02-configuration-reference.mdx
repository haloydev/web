...
| `api_token`           | object         | No       | API token configuration (see [Authentication](/docs/authentication))                                            |
...
| `targets`             | object         | No       | Multiple deployment targets with overrides (see [Multi-Server Deployments](/docs/multi-server-deployments)) |
| `secret_providers`    | object         | No       | Secret provider configuration (see [Secret Providers](/docs/secret-providers))                                    |
...
# Targets
  targets:
    production: # Inherits: replicas=2, port="8080" # Overrides: image.tag, adds domains, overrides env completely
      server: "prod.haloy.com"
      image:
        tag: "v1.2.3"
      domains:
      ...
      env: # Completely replaces base env
        - name: "NODE_ENV"
        value: "production"
    staging: # Inherits: image (repository + tag), replicas=2, port="8080", env # Overrides: server, adds domains, changes replicas
      server: "staging.haloy.com"
      replicas: 1
      domains:
