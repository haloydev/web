...
`haloy-proxy` listens on ports `80` and `443`, handles HTTP to HTTPS redirects, and serves TLS certificates requested by `haloyd`. Do not include a protocol, path, or port in `domain`; use only the hostname.
...
The `deployment_strategy` default is resolved per target from its final domain list: targets with domains default to `rolling`, targets without domains default to `replace`.
...
- `deployment_strategy 'rolling' requires at least one domain`: A target without domains is set to `rolling`, either directly or through a root `deployment_strategy: "rolling"`. Set `deployment_strategy: "replace"` on that target, or remove the root setting.
