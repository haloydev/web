...
description: 'Complete reference for all haloy.yaml configuration fields including server, domains, image, env, volumes, and deployment settings.'
---
...
A value source provides a string either directly with `value` or by reference with `from`. Use exactly one of these forms:
...
<Code code={`
  ...
  # From a configured secret provider
  from:
    secret: "sops:deploy-tokens:HALOY_API_TOKEN"
`} lang="yaml" />

Haloy uses this same shape for `api_token`, environment variable values, build arguments, and registry credentials. A `from` block must contain exactly one source, either `env` or `secret`.
...
| `naming_strategy`     | string         | No          | Container naming: "dynamic" (default) or "static" (see [Naming Strategy](#naming-strategy))                                                                                                                   |
| `image`               | string/object  | No          | Docker image configuration. A string is shorthand for `repository`. When omitted or partially specified, `repository` defaults to the target/app name. (see [Image Configuration](/docs/image-configuration)) |
...
| `api_token`           | value source   | No          | API token value source using `value`, `from.env`, or `from.secret` (see [Authentication](/docs/server-authentication))                                                                                        |
| `deployment_strategy` | string         | No          | Deployment strategy: "rolling" or "replace". Defaults to "rolling" when domains are set and "replace" otherwise. "rolling" requires at least one domain (see [Deployment Strategies](/docs/deployment-strategies)) |
| `domains`             | array          | No          | Domain routing configuration (see [Domain Configuration](/docs/domain-configuration))                                                                                                                         |
...
| `env`                 | array          | No          | Environment variables. Supports `${VAR_NAME}` interpolation in `value:` strings (see [Environment Variables](/docs/environment-variables))                                                                    |
...
- `image.pull_policy: "if_missing"` (reuse the local image when already present)
...
For practical multi-database and backup examples, see [Databases](/docs/databases).
...
- Requires `deployment_strategy: "replace"` (the default for targets without domains)
...
### Rolling Deployment (Default with domains)
...
### Replace Deployment (Default without domains)
...
| `api_token`           | value source  | Override the API token value source                  |
...
| `domains`             | array         | Override domain routing configuration                |
...
- The `deployment_strategy` default is resolved per target after inheritance: a target whose final configuration has no domains defaults to `replace`. A root `deployment_strategy: "rolling"` inherited by such a target fails validation, so set `replace` on that target instead
...
**Important**: If you override the default network (`haloy`), `haloyd` cannot resolve your containers' addresses and will not include them in the routing snapshot it pushes to `haloy-proxy`. This means:
...
- [Domain Configuration](/docs/domain-configuration)
