...
Tags embedded in the string (e.g., `"nginx:1.21"`) work the same way as in the `repository` field. All other image fields (`tag`, `registry`, `history`, `build`, `build_config`) require the object form.
