...
For credentials that should be available to haloyd on a server, use `haloy server registry login`. The CLI sends the credentials to the haloyd API, and haloyd uses them for future deploys and rollbacks that pull from that registry.
...
<Code code={`
  ...
  haloy server registry list haloy.example.com
`} lang="bash" />
