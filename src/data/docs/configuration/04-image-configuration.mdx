...
The `jobs` target has no domains, so it is not routed through the proxy and deploys with the `replace` strategy.
...
| `dockerfile` | string | No       | Path to Dockerfile, relative to context (default: "Dockerfile")     |
...
Paths are resolved the same way as in Docker Compose: the context is relative to the config file's directory, and the dockerfile is relative to the context.
...
<Code code={`
  ...
  # Shared Dockerfile outside the context
  build_config:
...
- A target without domains, like `jobs`, deploys with the `replace` strategy
