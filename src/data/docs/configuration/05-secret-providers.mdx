---
title: 'Secret Providers'
slug: 'secret-providers'
category: configuration

---

import Code from '@/components/code.astro'

Haloy integrates with external secret management services to securely manage credentials and sensitive data.

## 1Password Integration

Configure 1Password as a secret provider to pull secrets during deployment.

### Prerequisites

- 1Password CLI (`op`) installed and authenticated
- The 1Password vault and item must exist with the referenced field names

### Installation

Install the 1Password CLI:

<Code code={`# macOS
brew install 1password-cli

# Linux
curl -sSfo op.zip https://downloads.1password.com/linux/tar/stable/x86_64/op.zip
unzip op.zip && sudo mv op /usr/local/bin/

# Verify installation
op --version`} lang="bash" />

### Authentication

Sign in to 1Password CLI:

<Code code={`op account add
op signin`} lang="bash" />

### Configuration

Define secret sources in your `haloy.yaml`:

<Code code={`name: "my-app"
image:
  repository: "ghcr.io/your-username/my-app"
  tag: "latest"

# Configure 1Password secret sources
secretProviders:
  onepassword:
    production-db:  # Source name referenced in env vars
      account: "my-account"  # Optional: 1Password account
      vault: "Production"
      item: "Database Credentials"
    api-keys:
      vault: "API Services"
      item: "Third-party APIs"

env:
  - name: "DB_PASSWORD"
    from:
      secret: "onepassword:production-db.password"  # References vault item field
  - name: "DB_USERNAME"
    from:
      secret: "onepassword:production-db.username"
  - name: "STRIPE_API_KEY"
    from:
      secret: "onepassword:api-keys.stripe-key"
  - name: "SENDGRID_API_KEY"
    from:
      secret: "onepassword:api-keys.sendgrid-key"`} lang="yaml" />

### Secret Reference Format

<Code code={`onepassword:<source-name>.<field-name>`} lang="yaml" />

- `<source-name>`: The key defined in `secretProviders.onepassword`
- `<field-name>`: The field name in the 1Password item

### 1Password Item Structure

Your 1Password items should have fields matching your references:

**Example Item: "Database Credentials"**
- Field: `username` → Value: `db_user`
- Field: `password` → Value: `super_secret_password`
- Field: `host` → Value: `db.example.com`

**Example Item: "Third-party APIs"**
- Field: `stripe-key` → Value: `sk_live_...`
- Field: `sendgrid-key` → Value: `SG....`

## Registry Authentication with Secrets

Use 1Password to store registry credentials:

<Code code={`name: "my-app"
image:
  repository: "ghcr.io/your-org/private-app"
  tag: "latest"
  registry:
    username:
      from:
        secret: "onepassword:registry-credentials.username"
    password:
      from:
        secret: "onepassword:registry-credentials.password"

secretProviders:
  onepassword:
    registry-credentials:
      vault: "Infrastructure"
      item: "GitHub Container Registry"`} lang="yaml" />

## API Token with Secrets

Store your Haloy API token in 1Password:

<Code code={`name: "my-app"
server: "api.haloy.dev"
api_token:
  from:
    secret: "onepassword:api-tokens.production"

secretProviders:
  onepassword:
    api-tokens:
      vault: "Infrastructure"
      item: "Haloy API Tokens"`} lang="yaml" />

## Build Arguments with Secrets

Pass secrets to Docker build as build arguments:

<Code code={`name: "my-app"
image:
  repository: "my-app"
  tag: "latest"
  builder:
    context: "."
    args:
      - name: "NPM_TOKEN"
        from:
          secret: "onepassword:build-secrets.npm-token"
      - name: "GITHUB_TOKEN"
        from:
          secret: "onepassword:build-secrets.github-token"

secretProviders:
  onepassword:
    build-secrets:
      vault: "Development"
      item: "Build Tokens"`} lang="yaml" />

## Multi-Target with Different Secrets

Use different secrets for different deployment targets:

<Code code={`name: "my-app"
secretProviders:
  onepassword:
    prod-db:
      vault: "Production"
      item: "Database"
    staging-db:
      vault: "Staging"
      item: "Database"

targets:
  production:
    server: "prod.haloy.com"
    env:
      - name: "DB_PASSWORD"
        from:
          secret: "onepassword:prod-db.password"

  staging:
    server: "staging.haloy.com"
    env:
      - name: "DB_PASSWORD"
        from:
          secret: "onepassword:staging-db.password"`} lang="yaml" />

## Complete Example

<Code code={`name: "secure-app"
server: "haloy.example.com"

# Define all secret sources
secretProviders:
  onepassword:
    production-db:
      vault: "Production"
      item: "PostgreSQL Database"
    external-apis:
      vault: "Production"
      item: "Third-party APIs"
    infrastructure:
      vault: "Infrastructure"
      item: "Haloy & Registry"

# API authentication
api_token:
  from:
    secret: "onepassword:infrastructure.haloy-api-token"

# Image configuration with registry auth
image:
  repository: "ghcr.io/my-org/secure-app"
  tag: "latest"
  registry:
    username:
      from:
        secret: "onepassword:infrastructure.github-username"
    password:
      from:
        secret: "onepassword:infrastructure.github-token"

# Application environment variables
env:
  - name: "DATABASE_URL"
    from:
      secret: "onepassword:production-db.connection-url"
  - name: "DB_PASSWORD"
    from:
      secret: "onepassword:production-db.password"
  - name: "STRIPE_SECRET_KEY"
    from:
      secret: "onepassword:external-apis.stripe-secret"
  - name: "SENDGRID_API_KEY"
    from:
      secret: "onepassword:external-apis.sendgrid-key"

domains:
  - domain: "secure-app.com"
acme_email: "admin@secure-app.com"`} lang="yaml" />

## Validation

Validate your configuration and verify secrets are resolved correctly:

<Code code={`# Validate config (doesn't show secret values)
haloy validate-config

# Show resolved config with secrets (use with caution!)
haloy validate-config --show-resolved-config`} lang="bash" />

> **Warning:** `--show-resolved-config` displays all secrets in plain text. Only use in secure environments.

## Security Best Practices

1. **Never commit secrets**: Keep secrets in 1Password, not in your config files
2. **Use field-level access**: Reference specific fields rather than entire items
3. **Rotate secrets regularly**: Update secrets in 1Password, redeploy to apply
4. **Limit access**: Use 1Password's access controls to limit who can view secrets
5. **Audit secret usage**: Review 1Password access logs regularly
6. **Use separate vaults**: Different vaults for different environments (production, staging, development)

## Troubleshooting

### 1Password CLI Not Authenticated

<Code code={`# Check authentication status
op account list

# Sign in if needed
op signin`} lang="bash" />

### Secret Not Found

<Code code={`# Verify the item exists
op item get "Database Credentials" --vault "Production"

# List item fields
op item get "Database Credentials" --vault "Production" --fields label`} lang="bash" />

### Permission Denied

Ensure your 1Password account has access to the specified vault and item.

## Next Steps

- [Configure Environment Variables](/docs/environment-variables)
- [Set up Image Configuration](/docs/image-configuration)
- [Learn about Authentication](/docs/authentication)
