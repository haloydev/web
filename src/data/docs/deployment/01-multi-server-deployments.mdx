...
<Code code={`# Deploy to production only
haloy deploy --target production

# Short form
haloy deploy -t staging`} lang="bash" />
...
<Code code={`haloy deploy
# Shows available targets for selection`} lang="bash" />
...
<Code code={`haloy deploy --target production  # Correct
haloy deploy --target prod         # Wrong if target is named "production"`} lang="bash" />
