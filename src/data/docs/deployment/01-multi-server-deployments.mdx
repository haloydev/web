...
- A root `deployment_strategy: "rolling"` inherited by a target without domains fails validation. Set `replace` on that target, or leave the root value unset so each target gets its own default
...
- Other arrays (like `domains`, `volumes`) are replaced entirely, not merged. See [Domain Configuration](/docs/domain-configuration) for domain examples.
