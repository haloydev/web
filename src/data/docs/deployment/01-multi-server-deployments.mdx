...
section: deployment
description: 'Deploy your application to multiple servers with a single haloy.yaml. Configure multi-environment setups and geographic distribution.'
---
...
- Readiness settings like `min_ready_seconds` inherit the same way as `replicas` or `deployment_strategy`
...
staging:
server: "staging.haloy.com"
image: "ghcr.io/my-org/my-app:main"
min_ready_seconds: 15
domains: - domain: "staging.my-app.com"
`} lang="yaml" />

`production` inherits the 5-second stabilization window. `staging` overrides it to 15 seconds.
...
global_pre_deploy: - "echo 'Starting deployment pipeline'" - "npm run build"
global_post_deploy: - "echo 'All deployments completed'" - "curl -X POST https://status.io/webhook"
targets:
production:
server: prod.haloy.com
image:
...
global_post_deploy: - "npm run cleanup" - "echo 'All deployments complete'"
...
global_pre_deploy: - "npm run test"
