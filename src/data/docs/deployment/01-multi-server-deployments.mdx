...
<Code
  ...
  targets:
    production:
      ...
      # Overrides: image.tag, adds domains, replaces env
      ...
      env:
        ...
        - name: "LOG_LEVEL"
          value: "warn"
    staging:
      # Inherits: image (repo + tag), port="8080", env
...
- Target values completely override base values (no merging)
- Only specified fields in targets override the base
- Arrays (like `env`) are replaced entirely, not merged
...
pre_deploy: - "npm run migrate:production"
post_deploy: - "echo 'Production deployed'"
...
replicas: 1
env: - name: "NODE_ENV"
value: "development" - name: "DEBUG"
value: "true" - name: "DATABASE_URL"
from:
env: "DEV_DATABASE_URL"`} lang="yaml" />

## Geographic Distribution

Deploy to multiple regions:

<Code code={`name: "my-api"
image:
...
replicas: 2
env: - name: "REGION"
value: "us-east-1" - name: "DATABASE_URL"
from:
secret: "onepassword:us-db.url"

eu-west:
server: eu-west.haloy.com
domains: - domain: "eu-api.my-app.com"
replicas: 2
env: - name: "REGION"
value: "eu-west-1" - name: "DATABASE_URL"
from:
secret: "onepassword:eu-db.url"

asia-pacific:
server: ap-southeast.haloy.com
domains: - domain: "ap-api.my-app.com"
replicas: 2
env: - name: "REGION"
value: "ap-southeast-1" - name: "DATABASE_URL"
from:
secret: "onepassword:ap-db.url"`} lang="yaml" />
...
server: prod.haloy.com
staging:
server: staging.haloy.com`} lang="yaml" />
...
server: staging.haloy.com
pre_deploy: - "npm run migrate:staging"
post_deploy: - "npm run test:e2e:staging"`} lang="yaml" />

## Target-Specific Commands

All commands support target selection:

<Code code={`# Check status of specific target
...
haloy status --all
haloy stop --all`} lang="bash" />

## Managing Multiple Servers

Add all your servers to the local client:

<Code code={`# Add production server
...
haloy server delete staging.haloy.com`} lang="bash" />
...
image:
builder:
push: "server" # Upload to production server

staging:
server: staging.haloy.com
image:
builder:
push: "server" # Upload to staging server`} lang="yaml" />
...
<Code
  code={`haloy deploy --target production  # Correct
...
- [Manage Authentication](/docs/authentication)
