...
section: deployment
description: 'Choose between rolling deployments for zero-downtime updates and replace deployments for simpler cutover. Configure health checks, readiness stabilization, and timeouts.'
---

import Code from '@/components/code.astro';

Haloy supports two deployment strategies: rolling deployment, the default for targets with domains, and replace deployment, the default for targets without domains.

## Rolling Deployment (Default with domains)

Gradually replaces old containers with new ones, ensuring zero downtime.

Rolling is the default for targets that have at least one domain, and it requires one. The cutover happens in the proxy, which routes by domain: new containers are added to the route once healthy, then old containers are removed. A target without domains has no route to hand over and no stable address under dynamic container names, so it uses the replace strategy instead. Setting `deployment_strategy: "rolling"` on a target without domains, directly or through the root config, fails validation.

### How It Works

1. Start new container(s)
2. Wait for health checks to pass and any configured `min_ready_seconds` window
...
<Code
  code={`
  name: "my-app"
  deployment_strategy: "rolling"  # Default when domains are set, can be omitted
  replicas: 3
  domains:
    - domain: "my-app.com"
`}
...
- **More confidence at cutover**: `min_ready_seconds` can catch crash-after-startup issues
...
4. Wait for health checks and any configured `min_ready_seconds` window
...
It is also the default, and the only supported strategy, for targets without domains: databases, queues, workers and other internal services that are reached by other containers over the `haloy` network rather than through the proxy. Pair it with `naming_strategy: "static"` (or the `database` and `service` presets, which set both) so the service keeps a fixed container name that other apps can use as its hostname.
...
- Internal services without domains (databases, queues, workers)
...
4. If health checks pass but deployment still waits, check `min_ready_seconds`
...
3. Keep `min_ready_seconds` only as high as needed
4. Pre-warm caches and connections on startup
5. Consider switching to rolling deployment (requires a domain)

## Best Practices

1. **Use rolling for domain-routed production apps**: Ensures high availability
...
6. **Use `min_ready_seconds` for flaky startups**: Useful when failures appear a few seconds after the first healthy response
7. **Set appropriate replica counts**: More replicas = smoother rolling deployments
