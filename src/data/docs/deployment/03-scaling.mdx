...
- Configures load balancing in `haloy-proxy`
...
`haloy-proxy` distributes traffic using round-robin by default, sending requests to each healthy container in turn.

### Traffic Flow

<Code
  code={`Internet → haloy-proxy (80/443) → Container 1 (8080)
                         → Container 2 (8080)
                         → Container 3 (8080)`}
  lang="bash"
/>
...
<Code code={`
  ...
  # haloy-proxy only routes to healthy containers
