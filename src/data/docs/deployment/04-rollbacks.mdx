...
<Code code={`# List all available rollback points
...
haloy rollback --config path/to/config.yaml dep_abc123xyz

# Rollback specific target
haloy rollback --target production dep_abc123xyz`} lang="bash" />
