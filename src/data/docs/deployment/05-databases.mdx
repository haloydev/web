...
A database has no domains, so other containers reach it by its static container name rather than through the proxy. Targets without domains always use the `replace` strategy.
