...
section: examples
description: 'Deploy a TanStack Start app with a self-hosted PostgreSQL database to your own server using Haloy.'
---
...
For generic database target patterns and a reusable backup approach, see [Databases](/docs/databases).
...
<Code code="pnpm add -D vite @vitejs/plugin-react typescript @types/react @types/react-dom @types/node" lang="bash" />
...
Create `.env` for local development. This guide expects PostgreSQL to be available on `localhost:5432`. If you use different credentials, a different port, or a different host, adjust the connection string accordingly.
...
- Sets up credentials and a database matching your `DATABASE_URL`
...
The top-level `POSTGRES_*` variables are shared across both targets so the database container configuration and the application's connection string stay in sync.
...
2.  `tanstack-start-postgres`:
    ...
    - `DATABASE_URL` is composed with Haloy env interpolation and uses `postgres` as the internal hostname for the database target.
...
<Code code={`
  ...
  haloy tunnel --port 5432 -t postgres
`} lang="bash" />
