...
Haloy supports secret providers like 1Password and SOPS to securely manage credentials. Here is a 1Password example:
...
<Code code={`
  ...
  env:
    ...
    - name: DJANGO_SECRET_KEY
      from:
        secret: "onepassword:django-app:secret_key"
    - name: DJANGO_ALLOWED_HOSTS
      value: "my-app.example.com,localhost,127.0.0.1"
    - name: DATABASE_PATH
      value: "/app/data/db.sqlite3"
    ...
    - name: DJANGO_SUPERUSER_PASSWORD
      from:
        secret: "onepassword:django-app:superuser_password"
  volumes:
    - "db-data:/app/data"
`} lang="yaml" />

**Prerequisites for this 1Password example:**
- 1Password CLI (`op`) installed and authenticated
- Secrets stored in a 1Password vault with the referenced field names

SOPS-backed secrets are also supported. See the [Secret Providers](/docs/secret-providers) documentation for complete setup instructions.
