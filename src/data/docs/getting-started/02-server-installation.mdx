...
<Code code={`API_DOMAIN=api.example.com ACME_EMAIL=admin@example.com \\
  curl -fsSL https://sh.haloy.dev/install-haloyd.sh | sh`} lang="bash" />
...
<Code code="INSTALL_DOCKER=true curl -fsSL https://sh.haloy.dev/install-haloyd.sh | sh" lang="bash" />

### Option 2: Standalone Docker install script

Install Docker separately using the standalone script:

<Code code="curl -fsSL https://sh.haloy.dev/docker-install.sh | sh" lang="bash" />
