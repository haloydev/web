...
description: 'Install and configure the haloyd server daemon on your Linux server. Handles Docker orchestration, HTTPS certificates, and deployments.'
---
...
<Code code="journalctl -u haloyd -n 50" lang="bash" />

Common issues:
- Docker not running: `systemctl start docker`
- Port 80/443 in use: Check with `ss -tlnp | grep -E ':80|:443'`
...
To upgrade an existing `haloyd` installation, use one of the dedicated upgrade methods described in [Upgrading](/docs/upgrading). Do not re-run the install script to upgrade, as it will overwrite your systemd/OpenRC service file and discard any manual customizations you've made to it.
