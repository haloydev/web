...
description: 'Install and configure the Haloy server daemons on your Linux server. Handles Docker orchestration, HTTPS certificates, proxy routing, and deployments.'
---
...
This domain is for the Haloy server API. Application domains are configured separately in `haloy.yaml`; see [Domain Configuration](/docs/domain-configuration).

### 2. Configure haloy

<Code code={`haloyd config set api-domain YOUR_DOMAIN
systemctl restart haloyd`} lang="bash" />
...
<Code code="haloyd config get api-token" lang="bash" />
...
| `--skip-start` | `SKIP_START=true` | Don't start the services after installation |
...
curl -fsSL -o /usr/local/bin/haloy-proxy https://releases.haloy.dev/haloy-proxy-linux-amd64
chmod +x /usr/local/bin/haloyd
chmod +x /usr/local/bin/haloy-proxy
`} lang="bash" />
...
<Code code="setcap cap_net_bind_service=+ep /usr/local/bin/haloy-proxy" lang="bash" />
...
<Code code="haloyd init --api-domain haloy.yourserver.com" lang="bash" />
...
systemctl enable --now haloy-proxy haloyd
`} lang="bash" />
...
rc-update add haloy-proxy default
...
# Restart only the control plane; traffic keeps flowing through haloy-proxy
...
<Code code={`
journalctl -u haloyd -n 50
journalctl -u haloy-proxy -n 50
`} lang="bash" />
...
ss -tlnp | grep -E 'haloyd|haloy-proxy'
...
<Code code="127.0.1.1 myserver.example.com myserver" lang="text" />
...
<Code code="getent ahostsv4 myserver.example.com" lang="bash" />
...
<Code lang="bash" code={`
  ...
  # Remove the FQDN from the 127.0.1.1 line
`} />

Then restart haloyd:

<Code code="systemctl restart haloyd" lang="bash" />
...
To upgrade an existing server installation, use one of the dedicated upgrade methods described in [Upgrading](/docs/upgrading). Do not re-run the install script to upgrade, as it will overwrite your systemd/OpenRC service files and discard any manual customizations you've made to them.
