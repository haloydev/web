...
<Code code={`# Generate token on the server
...
haloy server add dev.haloy.com <dev-token>`} lang="bash" />
...
<Code code={`servers:
  ...
  "staging.haloy.com":
    token_env: "HALOY_API_TOKEN_STAGING_HALOY_COM"`} lang="yaml" />

### Environment Variables

**`~/.config/haloy/.env`** - Actual tokens:

<Code code={`HALOY_API_TOKEN_PRODUCTION_HALOY_COM=abc123token456
HALOY_API_TOKEN_STAGING_HALOY_COM=def789token012`} lang="bash" />
...
image:
  repository: "my-app"
  tag: "latest"`} lang="yaml" />

Set the token in your environment:

<Code code={`export PRODUCTION_DEPLOY_TOKEN="your_token_here"
haloy deploy`} lang="bash" />

### Legacy Style (Simplified)
...
<Code code={`# production.haloy.yaml
...
api_token:
  from:
    env: "STAGING_TOKEN"`} lang="yaml" />

Deploy to different environments:

<Code code={`export PROD_TOKEN="production_token_here"
export STAGING_TOKEN="staging_token_here"

haloy deploy --config production.haloy.yaml
haloy deploy --config staging.haloy.yaml`} lang="bash" />

### CI/CD with Multiple Projects

<Code code={`# Set tokens in CI environment variables
...
api_token:
  from:
    env: "PROJECT_B_TOKEN"`} lang="yaml" />

### Single Server, Multiple Projects

<Code code={`# All projects deploy to the same server
...
# Or specify different tokens if needed`} lang="yaml" />
...
# Don't commit local client config
.haloy/`} lang="bash" />
...
GITHUB_TOKEN=your_github_token_here`} lang="bash" />

## Server Domain Format

When adding servers:
- Use just the domain name (e.g., `haloy.example.com`)
- Don't include `https://` - Haloy adds it automatically
- For local development: `localhost` or `127.0.0.1`

**Valid:**
<Code code={`haloy server add haloy.example.com <token>
haloy server add 192.168.1.100 <token>
haloy server add localhost <token>`} lang="bash" />

**Invalid:**
<Code code={`haloy server add https://haloy.example.com <token>  # Remove https://
haloy server add haloy.example.com:443 <token>        # Remove port`} lang="bash" />
...
**Solution**:
<Code code={`# Generate new token on server
sudo haloyadm api token

# Update local config
haloy server add <server-domain> <new-token> --force`} lang="bash" />
...
**Solution**:
<Code code={`# List configured servers
haloy server list

# Add missing server
haloy server add <server-domain> <token>`} lang="bash" />
...
**Solution**:
<Code code={`# Check if variable is set
...
# Or add to .env file
echo "PRODUCTION_API_TOKEN=your_token" >> .env`} lang="bash" />
...
**Solution**:
<Code code={`# Add server to your haloy.yaml
server: production.haloy.com`} lang="yaml" />
