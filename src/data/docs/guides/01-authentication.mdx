...
<Code
  code={`HALOY_API_TOKEN_PRODUCTION_HALOY_COM=abc123token456
...
<Code
  code={`export PRODUCTION_DEPLOY_TOKEN="your_token_here"
haloy deploy`}
  lang="bash"
/>
...
<Code
  code={`haloy server add haloy.example.com <token>
...
<Code
  code={`haloy server add https://haloy.example.com <token>  # Remove https://
...
<Code
  code={`# Add server to your haloy.yaml
server: production.haloy.com`}
  lang="yaml"
/>
