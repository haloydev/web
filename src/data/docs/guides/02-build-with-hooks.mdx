...
section: guides
description: 'Use deployment hooks for custom build processes. Run local Docker builds and upload images to your Haloy server manually.'
---
  ...
  targets:
    production:
      ...
      pre_deploy:
        ...
        - "ssh deploy@prod-server \\"docker load -i /tmp/complex-app.tar && rm /tmp/complex-app.tar\\""
`} />
...
4. **Use a fixed image tag**: Ensure hooks build/load the exact image tag Haloy deploys
