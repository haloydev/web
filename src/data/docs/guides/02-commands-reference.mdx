...
<Code code={`# Deploy with default config
haloy deploy
...
haloy deploy --no-logs`} lang="bash" />
...
<Code code={`# Check status with default config
haloy status
...
haloy logs --target staging`} lang="bash" />
...
# PowerShell
haloy completion powershell`} lang="bash" />
