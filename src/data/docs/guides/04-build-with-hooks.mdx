...
For most use cases, use Haloy's [Image Build Config](/configuration/image-configuration#image-build-config) feature:
...
targets:
  ...
  staging:
    ...
    pre_deploy:
      ...
      - "ssh $(whoami)@staging-server-ip \\"docker load -i /tmp/my-app.tar && rm /tmp/my-app.tar\\""`} lang="yaml" />
...
targets:
  production:
    server: "prod.haloy.com"
    domains:
      - domain: "complex-app.com"
    pre_deploy:
      - "scp complex-app.tar deploy@prod-server:/tmp/"
      - "ssh deploy@prod-server \\"docker load -i /tmp/complex-app.tar && rm /tmp/complex-app.tar\\"`} lang="yaml" />
...
<Code code={`pre_deploy:
  ...
  - "ssh prod-server \\"docker load -i /tmp/my-app.tar\\""`} lang="yaml" />
...
<Code code={`pre_deploy:
  ...
  - "ssh server \\"docker load -i /tmp/app.tar && rm /tmp/app.tar\\""`} lang="yaml" />
...
<Code code={`pre_deploy:
  ...
  - "ssh server \\"docker load -i /tmp/my-app.tar\\""`} lang="yaml" />
...
<Code code={`post_deploy:
  - "curl -H \\"Content-Type: application/json\\" -d '{\\"content\\": \\"Deployment complete!\\"}' $DISCORD_WEBHOOK"`} lang="yaml" />

### Email Notifications

<Code code={`post_deploy:
  - "echo 'Deployment successful' | mail -s 'Deployment Status' admin@example.com"`} lang="yaml" />
