...
<Code
  code={`mkdir -p ~/.local/share/zsh/site-functions
...
<Code
  code={`echo 'fpath=(~/.local/share/zsh/site-functions $fpath)' >> ~/.zshrc
...
<Code
  code={`mkdir -p ~/.config/fish/completions
...
<Code
  code={`haloy completion powershell > haloy.ps1
...
<Code
  code={`haloy <tab><tab>      # Shows all available commands
