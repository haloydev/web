...
Execute commands inside running application containers. Useful for debugging, inspecting container state, or running one-off administrative commands.
...
<div class="list-unstyled">
- `--config, -c <path>` - Path to config file or directory (default: current directory)
- `--targets, -t <targets>` - Execute on specific targets (comma-separated)
...
<Code code={`
  # Basic setup
  haloy server setup <host>

  # Setup with API domain and email
  haloy server setup 192.168.1.100 --api-domain api.example.com --acme-email admin@example.com

  # Setup with specific user and identity file
...
- `--targets, -t` - Get version for specific targets (comma-separated)
...
<Code code={`
  # Set API domain with automatic SSL
...
Both `haloy` and `haloyadm` CLI tools use consistent exit codes:
